							...
							<?php if($full_width == true or ($full_width == false and $first_half == true)){ ?><div class="form-group"><?php } ?>
								...
								<div class="<?php echo $input_width; ?>">
									...
									<?php }elseif($tf['type'] == 'datepicker'){ ?>
										<div class="input-group">
											<div class="input-group-addon"><i class="fa fa-calendar"></i></div>
											<input type="text" class="form-control daterangepicker picker_3" id="<?php echo $tf['table_index'] ?>" name="<?php echo $tf['table_index'] ?>" value="<?php echo isset($datas[$tf['table_index']])? $datas[$tf['table_index']] : $tf['value'] ?>" <?php if($tf['required']){ ?> required="required" <?php } ?> />
										</div>
